...
export function ContentTypesManager({ initialData }: ContentTypesManagerProps) {
  const router = useRouter()
  const [items, setItems] = useState<ContentType[]>(initialData)
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      alert(err.message || 'Failed to add content type')
    } finally {
      setLoading(false)
    }
  }
  ...
  const openEditDialog = (item: ContentType) => {
    setEditingItem(item)
    setName(item.name)
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{editingItem ? 'Edit' : 'Add'} Content Type</DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="e.g., Post, Reel, Video"
                />
              </div>
